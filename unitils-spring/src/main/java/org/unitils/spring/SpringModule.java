...
public class SpringModule
            ...
            getDatabaseModule().registerTransactionManagementConfiguration(new UnitilsTransactionManagementConfiguration() {
                ...
                public PlatformTransactionManager getSpringPlatformTransactionManager(Object testObject) {
                    ...
                    if (platformTransactionManagers.size() > 1) {
                        ...
                        String transactionManagerName = getMethodOrClassLevelAnnotationProperty(Transactional.class, "transactionManagerName", "", testMethod,
                            testObject.getClass());
    ...
    public <T> T getSpringBeanByType(Object testObject, Class<T> type) {
        ...
        if (beans.size() > 1) {
            throw new UnitilsException(
                "Unable to get Spring bean by type. More than one possible Spring bean for type " + type.getSimpleName() + ". Possible beans; " + beans);
        }
    ...
    public void injectApplicationContext(Object testObject) {
        ...
        for (Field field : fields) {
            try {
                ...
                throw new UnitilsException(
                    "Unable to assign the application context to field annotated with @" + SpringApplicationContext.class.getSimpleName(), e);
            }
        }
        ...
        for (Method method : methods) {
            ...
            try {
                ...
                throw new UnitilsException(
                    "Unable to assign the application context to setter annotated with @" + SpringApplicationContext.class.getSimpleName(), e);
            }
        }
    }
    ...
    public void injectSpringBeans(Object testObject) {
        ...
        for (Method method : methods) {
            try {
                if (!isSetter(method)) {
                    throw new UnitilsException("Unable to assign the Spring bean value to method annotated with @" + SpringBean.class.getSimpleName()
                        + ". Method " + method.getName() + " is not a setter method.");
                }
    ...
    public void injectSpringBeansByType(Object testObject) {
        ...
        for (Method method : methods) {
            try {
                if (!isSetter(method)) {
                    throw new UnitilsException("Unable to assign the Spring bean value to method annotated with @" + SpringBeanByType.class.getSimpleName()
                        + ". Method " + method.getName() + " is not a setter method.");
                }
                ...
                throw new UnitilsException("Unable to assign the Spring bean value to method annotated with @" + SpringBeanByType.class.getSimpleName()
                    + ". Method " + "has thrown an exception.", e.getCause());
            }
        }
    }
    ...
    public void injectSpringBeansByName(Object testObject) {
        ...
        for (Method method : methods) {
            try {
                if (!isSetter(method)) {
                    throw new UnitilsException("Unable to assign the Spring bean value to method annotated with @" + SpringBeanByName.class.getSimpleName()
                        + ". Method " + method.getName() + " is not a setter method.");
                }
