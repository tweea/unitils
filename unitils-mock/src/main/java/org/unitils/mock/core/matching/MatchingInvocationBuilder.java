...
public class MatchingInvocationBuilder {
    ...
    public synchronized void assertNotExpectingInvocation() {
        if (currentMockName != null && !isChainedMock()) {
            UnitilsException exception = new UnitilsException("Invalid syntax. " + currentMockName + "." + definingMethodName
                + "() must be followed by a method invocation on the returned proxy. E.g. " + currentMockName + "." + definingMethodName + "().myMethod();");
