...
public class InjectionUtils {
     ...
     *     The target object
     * @param property
     *     The OGNL expression that defines where the object will be injected, not null
     ...
     *     The type of the object. This should be the type of the object or one of his super-types
     ...
     * @param isStatic
     *     Indicates wether injection should be performed on the target object or on the target class
     * @return The object that was replaced by the injection
     */
    private static Object injectIntoFieldByType(Object objectToInject, Type objectToInjectType, Object target, Class<?> targetClass, boolean isStatic) {
        ...
        if (fieldsWithExactType.size() > 1) {
            StringBuilder message = new StringBuilder(
                "More than one " + (isStatic ? "static " : "") + "field with type " + objectToInjectType + " found in " + targetClass.getSimpleName() + ".");
            ...
            if (fieldsOfType.size() == 0) {
                throw new UnitilsException(
                    "No " + (isStatic ? "static " : "") + "field with (super)type " + objectToInjectType + " found in " + targetClass.getSimpleName());
            }
    ...
    private static Object injectIntoSetterByType(Object objectToInject, Type objectToInjectType, Object target, Class<?> targetClass, boolean isStatic) {
        ...
        if (settersWithExactType.size() > 1) {
            throw new UnitilsException(
                "More than one " + (isStatic ? "static " : "") + "setter with type " + objectToInjectType + " found in " + targetClass.getSimpleName());
            ...
            if (settersOfType.size() == 0) {
                throw new UnitilsException(
                    "No " + (isStatic ? "static " : "") + "setter with (super)type " + objectToInjectType + " found in " + targetClass.getSimpleName());
            }
        ...
        try {
            ...
            if (getter == null) {
                logger.warn("Unable to retrieve current value of field to inject into, no getter found for setter: " + setterToInjectTo
                    + ". Will not be able to restore value after injection.");
            } else {
     ...
     *     The name of the property (simple name, not a composite expression)
