        ...
        for (int i = 0; i < filteredResources.size(); i++) {
            ...
            if (urlNew.toString().endsWith(endingWith)) {
                ...
                if (inlistresource < newresource) {
                    logger.debug("Resource replace by more recent after PickingStrategie: '" + url + "'(" + sdf.format(new Date(inlistresource))
                        + ") replaced_by '" + urlNew + "' (" + sdf.format(new Date(newresource)) + ")");
                    filteredResources.remove(url);
                    filteredResources.add(urlNew);
                } else {
                    logger.debug("Duplicate Resource found after PickingStrategie but not more recent (not added): '" + url + "'("
                        + sdf.format(new Date(inlistresource)) + ") is choosen over '" + urlNew + "' (" + sdf.format(new Date(newresource)) + ")");
                }
                return;
            }
        }
